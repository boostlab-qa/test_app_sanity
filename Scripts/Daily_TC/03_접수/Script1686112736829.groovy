...
'기대결과 - 병원상세 화면으로 이동'
Mobile.verifyElementVisible(findTestObject('00_common/txt_class_text_view', [('text') : GlobalVariable.hospital_name]), GlobalVariable.fixedTime, FailureHandling.CONTINUE_ON_FAILURE) //page title
...
'건강검진 미사용 병원인 경우'
if(Mobile.waitForElementPresent(findTestObject('09_hospital_detail/btn_left', [('text') : '바로접수']), GlobalVariable.fixedTime, FailureHandling.CONTINUE_ON_FAILURE) == true)
{
'STEP - [바로접수] 버튼 선택'
...
if(Mobile.waitForElementPresent(findTestObject('10_receipt/btn_deny'), GlobalVariable.fixedTime, FailureHandling.CONTINUE_ON_FAILURE))
{
	'STEP - [다음에 하기] 버튼 선택'
...
'기대결과 - 홈 화면으로 이동'
Mobile.verifyElementVisible(findTestObject('03_home/btn_menu_home'), GlobalVariable.fixedTime, FailureHandling.CONTINUE_ON_FAILURE)
